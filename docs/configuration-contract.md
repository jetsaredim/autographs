...
The tenancy bootstrap root also creates a default OCI Vault, a software-protected runtime secrets key, and a disposable `autographs-runtime-vault-proof` secret with OCI-generated content for the instance-principal retrieval proof. It grants the runtime dynamic group read access to OCI Vault secret bundles in the Autographs project compartment. This is intentionally compartment-scoped because the project compartment is single-purpose for Autographs resources; do not place unrelated Vault secrets in that compartment without first splitting the compartment model or tightening the policy to specific secret OCIDs.
