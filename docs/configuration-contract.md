...
secret for the database update. Before regional Vault resources move into the
runtime root, the tenancy root must be applied so its deploy policy grants
compartment-scoped management of vaults, keys, and the secret family. This
permission is limited to the dedicated Autographs project compartment rather
than the tenancy. It replaces the former metadata-only lookup and
database-password-only bundle policy because the deployment root will own the
full lifecycle of those regional resources.
