...
Replace the current DNF updateinfo-based production security scanner with an OpenSCAP/Oracle OVAL finding source, preserve GitHub issue approval semantics with advisory IDs, and keep remediation scoped to approved DNF advisory updates while reporting Ksplice availability.
...
2. Apply and result behavior - completed
   ...
   - Detect Ksplice availability, apply approved advisories with DNF advisory filters, and re-scan again.
   - Report Ksplice availability, DNF update signals, and remaining OVAL findings in the result comment.
