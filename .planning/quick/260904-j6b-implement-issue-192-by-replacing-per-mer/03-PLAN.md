...
<task type="auto">
  ...
  <action>
    Add `docs/release-management.md` as the canonical operator procedure and link it from README/deployment docs. Describe the conventional commit contract, accumulating ready Release PR, `always-update`, cumulative CI, and exact GitHub App creation/installation/key-rotation boundary: Autographs repository only, Contents read/write, Pull requests read/write, hour-lived installation tokens, and no OCI/package/tenancy permissions (D-01, D-12, D-15-D-20). State that action errors always stop and that any unresolved draft blocks release-please until the operator manually dispatches retry for that exact tag (D-20-D-21).
    ...
    Replace README's generated per-merge status block with stable links to GitHub Releases and `.release-status.json`. Correct every claim that ordinary merges tag/deploy, that digests are operator-facing deploy references, or that repo tags imply controller tags. Update configuration fields for active/previous tag/digest and separate cleanup controls. Explain local cleanup, scheduled remote inventory, explicit manual GHCR deletion, and the pre-prune GitHub Release/source-tag/GHCR inventory that names current deployed and rollback-protected artifacts. Update dependency docs for both privileged action SHAs and the GitHub env example with `RELEASE_PLEASE_APP_CLIENT_ID`, `RELEASE_PLEASE_APP_PRIVATE_KEY`, and cleanup controls. Update public readiness so the initial implementation and future Release PRs are ready PRs with CI/review evidence (D-10, D-13-D-14, D-24).
  </action>
  <verify>
...
8. After the implementation PR merge, verify it only opens/updates the release-please Release PR. Install and configure the release GitHub App before that merge. After Release PR CI passes, merge it and validate the expected bootstrap: a new repo tag/draft appears, the classified release selects or builds the correct controller tag, full production deploy/health and status commit succeed, and only then the Release is published. Run an incremental publish.
</verification>
