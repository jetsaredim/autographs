...
- Changed `INT`/`TERM` handling to terminate with conventional signal exit statuses and flow through the `EXIT` cleanup instead of returning to the gate after cleanup.

Repository hygiene, extracted Bash syntax, signal-termination behavior, and diff checks pass. The candidate gate remains an operator-run VM check and is intentionally still required before PR #211 can merge.
