...
| 3 | Operators have a safe pre-merge migrated-controller gate. | VERIFIED | The runbook builds the exact candidate, starts it alongside production with copied wallet/secrets, targets it from the established static-publish smoke, defines success, and uses a strict subshell, `EXIT` cleanup, and terminating `INT`/`TERM` handlers for mandatory cleanup on every outcome. The extracted Bash block passes `bash -n`, and a signal simulation exits 130 after cleanup without continuing. |
