...
- [Phase 08]: Production security remediation keeps Ksplice report-only, applies exact approved advisories with `dnf upgrade-minimal --security --advisories`, re-scans with OpenSCAP, and routes remaining findings to update, reboot, investigation, or clean closure from authoritative scan state.
...
| 2026-08-03 | replace-production-security-patch-scanne | Replaced the production security scanner with OpenSCAP Oracle OVAL findings, advisory-ID approval metadata, report-only Ksplice detection, and DNF advisory-scoped updates. |
