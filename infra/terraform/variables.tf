...
variable "create_public_dns_zone" {
  description = "Whether Terraform should create the OCI public DNS zone and app A record."
  type        = bool
  default     = false
}

variable "public_dns_zone_name" {
  description = "OCI public DNS zone name delegated from the parent registrar zone."
  type        = string
  default     = "autographs.jetsaredim.net"

  validation {
    condition     = can(regex("^[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?(\\.[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?)+$", var.public_dns_zone_name))
    error_message = "public_dns_zone_name must be a valid DNS name without a trailing dot."
  }
}

variable "public_dns_record_name" {
  description = "FQDN for the public app A record. For subdomain delegation this is usually the zone apex."
  type        = string
  default     = "autographs.jetsaredim.net"

  validation {
    condition     = can(regex("^[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?(\\.[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?)+$", var.public_dns_record_name))
    error_message = "public_dns_record_name must be a valid DNS name without a trailing dot."
  }
}

variable "public_dns_record_ttl" {
  description = "TTL in seconds for the public DNS A record."
  type        = number
  default     = 300

  validation {
    condition     = var.public_dns_record_ttl >= 30 && var.public_dns_record_ttl <= 86400
    error_message = "public_dns_record_ttl must be between 30 and 86400 seconds."
  }
}
