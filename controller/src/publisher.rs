...
fn browse_script() -> &'static str {
    "Promise.all([fetch('/data/collection.json').then(r=>r.json()),fetch('/data/facets.json').then(r=>r.json())]).then(([catalog,facets])=>{const root=document.querySelector('#collection');const category=document.querySelector('#category');const tag=document.querySelector('#tag');const options=(id)=>facets.groups.find(g=>g.id===id).options;const fill=(node,values)=>{node.innerHTML='<option value=\"\">All</option>'+values.map(v=>`<option>${v.value}</option>`).join('')};fill(category,options('category'));fill(tag,options('tag'));const render=()=>{root.innerHTML=catalog.items.filter(i=>(!category.value||i.category===category.value)&&(!tag.value||i.tags.includes(tag.value))).map(i=>`<article><a href=\"/items/${i.slug}/\">${i.title}</a><p>${i.signer}</p></article>`).join('')};category.onchange=render;tag.onchange=render;render()})"
}
