...
create table autograph_items (
  ...
  format varchar2(80) default 'Trading Card' not null,
  origin varchar2(24) default 'Official' not null,
  language varchar2(40) default 'English' not null,
  ...
  constraint autograph_items_format_ck
    check (trim(format) is not null),
  constraint autograph_items_origin_ck
    check (origin in ('Official', 'Custom')),
  constraint autograph_items_language_ck
...
create index autograph_items_format_idx on autograph_items(format);

create index autograph_items_origin_idx on autograph_items(origin);

create index autograph_items_language_idx on autograph_items(language);
...
create index autograph_item_signers_signer_idx
  on autograph_item_signers(signer_id);

create index autograph_item_franchises_value_idx
  on autograph_item_franchises(franchise);
